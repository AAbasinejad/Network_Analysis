        ...
        for auth in inf_data[i]["authors"]:
            if auth["author_id"] not in graph.nodes():
                graph.add_node(auth["author_id"], 
                               name=f.clean_name(auth["author"]), 
                               id_publication_int=list(diz_aut[auth["author_id"]]['id_publication_int']), 
                               id_conference_int=list(diz_aut[auth["author_id"]]['id_conference_int']))
